...
$DatosRespuesta['norecibo'] 	= '0';
$DatosRespuesta['FechaRecibo'] 	= '0';
...
if ($result->num_rows > 0) {
	...
	if ($SaldoCli == 0) {
		...
		$Q = "
		SELECT
			c.barriada, b.nombre NombreBarriada, c.cobrador, DATE_FORMAT(NOW(), c.contrasena
		FROM
			contratos c
		INNER JOIN 
			barriadas b on c.barriada = b.idbarriada
		WHERE
		...
		if ($result_4->num_rows > 0) {
			while ($row_4 = $result_4->fetch_assoc()) {
				...
				$NombreBarriada = $row_4['NombreBarriada'];
				$Cobrador 		= $row_4['cobrador'];
				$contrasena 	= $row_4['contrasena'];
			}
		}
		...
		$sql = "
		SELECT 
			DATE_FORMAT(fecha, '%d/%m/%Y %h:%i') FechaRecibo 
		FROM 
			recibo 
		WHERE 
			num_rec='" . $NumRec . "' AND num_contrato='" . $IdContrato . "' AND tipo_cli='" . $TipoContrato . "';";
		$result_3 = $db->query($sql);
		if ($result_3->num_rows > 0) {
			$row_3 			= $result_3->fetch_assoc();
			$FechaRecibo 	= $row_3['FechaRecibo'];
		}

		$DatosRespuesta['error'] 		= 0;
		$DatosRespuesta['codigo'] 		= 0;
		$DatosRespuesta['barriada']		= $NombreBarriada;
		$DatosRespuesta['descripcion'] 	= $TXT1m;
		$DatosRespuesta['norecibo'] 	= $NumRec;
		$DatosRespuesta['FechaRecibo'] 	= $FechaRecibo;
		...
		$Q = "
		SELECT
			t.num_doc, DATE(t.fecha) fecha, t.db, t.saldo, UPPER(DATE_FORMAT(t.fecha, '%b-%Y')) txt1,
			c.barriada, b.nombre NombreBarriada, c.cobrador, c.contrasena
		FROM
			transacciones t
		INNER JOIN
		...
		if ($result_4->num_rows > 0) {
			...
			$DatosRespuesta['norecibo'] 	= $NumRec;
			$DatosRespuesta['FechaRecibo'] 	= $FechaRecibo;
			$DatosRespuesta['contrasena']	= $contrasena;
			$DatosRespuesta['mensaje'] 	= 'Se ha realizado el PAGO correctamente';
		} else {
			$DatosRespuesta['error'] 	= 1;
